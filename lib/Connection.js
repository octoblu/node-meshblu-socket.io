...
Connection.prototype.getPublicKey = function(uuid, callback) {
  var self = this;
  this.device({uuid: uuid}, function(err, data){
    if (err) {
    ...
    if (!data.device.publicKey) {
      return callback(new Error('Device has no public key'))
    }

    callback(null, new self.NodeRSA(data.device.publicKey) );
...
Connection.prototype.encryptMessage = function(uuid, message, fn) {
  var self = this;
  self.getPublicKey(uuid, function(error, key){
    ...
    self.message(uuid, undefined, encryptedMessage,fn);
  });
};

Connection.prototype.message = function() {
  var devices, message, fn

  if (arguments[0].devices) {
    message = arguments[0];
    fn = arguments[1];
    return this._emitWithAck('message', message, fn);
  }

  message = { devices: arguments[0], payload: arguments[1] };
  fn = arguments[2];

  if ( arguments[3] instanceof Function) {
    _.extend(message, arguments[2]);
    fn = arguments[3];
  }

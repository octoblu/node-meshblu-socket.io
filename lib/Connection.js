...
Connection.prototype.encryptMessage = function(data, fn) {
  var self = this;
  self.getPublicKey(data.devices, function(error, key){
    if (error) {
      self.console.error("can't find public key for device");
      return;
    }
    data.encryptedPayload = key.encrypt(JSON.stringify(data.encryptedPayload)).toString('base64');
    self.message(data,fn);
  });
};
